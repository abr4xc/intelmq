...
After a match, then the abuse contact will be fetched from the trusted introducer teams list and will be stored in the event as `source.abuse_contact`.
If there is a no match, the event will not be enriched and will be sent to the next configured step.
