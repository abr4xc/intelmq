...
class JinjaExpertBot(Bot):
    ...
    def init(self):
        if not Template:
            raise ValueError("Library 'jinja2' is required, please install it.")
        ...
        for field, template in self.fields.items():
            try:
                ...
                raise ValueError(f"Error parsing Jinaj Template for '{field}': {msg}")

    def process(self):
        msg = self.receive_message()

        for field, template in self.fields.items():
            msg.add(field, template.render(msg=msg), overwrite=True)
